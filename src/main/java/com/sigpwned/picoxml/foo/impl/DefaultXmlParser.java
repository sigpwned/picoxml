...
import com.sigpwned.picoxml.foo.CDATAReader;
import com.sigpwned.picoxml.foo.ContentReader;
import com.sigpwned.picoxml.foo.IXMLBuilder;
import com.sigpwned.picoxml.foo.IXMLEntityResolver;
import com.sigpwned.picoxml.foo.IXMLParser;
import com.sigpwned.picoxml.foo.IXMLReader;
import com.sigpwned.picoxml.foo.IXMLValidator;
import com.sigpwned.picoxml.foo.PIReader;
import com.sigpwned.picoxml.foo.XMLEntityResolver;
import com.sigpwned.picoxml.foo.XMLException;
import com.sigpwned.picoxml.foo.XMLUtil;


/**
 * DefaultXmlParser is the core parser of NanoXML.
 *
 */
public class DefaultXmlParser implements IXMLParser {
  ...
  protected void scanData() throws Exception {
    while ((!this.reader.atEOF()) && (this.builder.getResult() == null)) {
      ...
      switch (ch) {
        ...
        default:
          XMLUtil.errorInvalidInput(reader.getSystemID(), reader.getLineNr(),
              "`" + ch + "' (0x" + Integer.toHexString(ch) + ')');
      }
    }
  }
  ...
  protected void processElement(String defaultNamespace, Properties namespaces) throws Exception {
    ...
    this.validator.elementAttributesProcessed(fullName, extraAttributes, this.reader.getSystemID(),
        this.reader.getLineNr());
    Enumeration iter = extraAttributes.keys();
    ...
    if (prefix == null) {
      this.builder.startElement(name, prefix, defaultNamespace, this.reader.getSystemID(),
          this.reader.getLineNr());
    } else {
      this.builder.startElement(name, prefix, namespaces.getProperty(prefix),
          this.reader.getSystemID(), this.reader.getLineNr());
    }
